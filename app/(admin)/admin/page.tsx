...
import { DollarSign, TrendingUp, BookOpen, AlertTriangle, ArrowRight, Plus } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400">Real-time dropshipping business operations and catalog status.</p>
        </div>
      ...
      {/* 4 KPI Cards: 1 column on mobile, 2 on tablet, 4 on desktop */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        
        {/* Card 1: Catalog Value (Available Books Total Price) */}
        <Card className="bg-slate-950 border-slate-800">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-xs font-bold uppercase tracking-wider text-slate-500">Catalog Value</CardTitle>
            <div className="bg-emerald-500/10 p-2 rounded-lg text-emerald-400"><DollarSign className="w-4 h-4" /></div>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold font-mono text-emerald-400">GH₵{data.catalogValue.toFixed(2)}</div>
            <p className="text-[10px] text-slate-500 mt-1">Total available book prices</p>
          </CardContent>
        </Card>

        {/* Card 2: Profit Potential (Available Books Margin Sum) */}
        <Card className="bg-slate-950 border-slate-800">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-xs font-bold uppercase tracking-wider text-slate-500">Profit Potential</CardTitle>
            <div className="bg-amber-500/10 p-2 rounded-lg text-amber-400"><TrendingUp className="w-4 h-4" /></div>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold font-mono text-amber-400">GH₵{data.profitPotential.toFixed(2)}</div>
            <p className="text-[10px] text-slate-500 mt-1">Sum of all profit margins</p>
          </CardContent>
        </Card>

        {/* Card 3: Available Books Count */}
        <Card className="bg-slate-950 border-slate-800">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-xs font-bold uppercase tracking-wider text-slate-500">Available Books</CardTitle>
            <div className="bg-blue-500/10 p-2 rounded-lg text-blue-400"><BookOpen className="w-4 h-4" /></div>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-slate-100 font-mono">{data.availableCount} / {data.count}</div>
            <p className="text-[10px] text-slate-500 mt-1">In stock with supplier</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-slate-950 border-slate-800">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-xs font-bold uppercase tracking-wider text-slate-500">Out of Stock</CardTitle>
            <div className="bg-rose-500/10 p-2 rounded-lg text-rose-400"><AlertTriangle className="w-4 h-4" /></div>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-rose-400 font-mono">{data.outOfStock.length}</div>
            <p className="text-[10px] text-slate-500 mt-1">Marked unavailable</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Out of Stock Books List Column */}
        <Card className="bg-slate-950 border-slate-800 flex flex-col justify-between">
          <div>
            <CardHeader className="border-b border-slate-900 p-4 flex flex-row items-center justify-between">
              <CardTitle className="text-base font-bold text-slate-200">Unavailable Books</CardTitle>
              <AlertTriangle className="w-4 h-4 text-rose-400" />
            </CardHeader>
            <CardContent className="p-4 flex-grow">
              {data.outOfStock.length === 0 ? (
                <div className="text-xs text-slate-500 text-center py-12">All books are marked as available.</div>
              ) : (
                <div className="space-y-3">
                  {data.outOfStock.map((b) => (
                    <div key={b.id} className="p-3 bg-slate-900 rounded-lg border border-slate-800/80 flex items-center justify-between gap-3">
                      ...
                      <span className="text-[10px] font-mono font-bold px-2 py-0.5 bg-rose-500/10 text-rose-400 rounded border border-rose-500/20 shrink-0 whitespace-nowrap">Out</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="p-4 border-t border-slate-900">
            {/* Navigate to Inventory Management to reactivate books */}
            <Button asChild className="w-full bg-slate-900 hover:bg-slate-850 text-slate-200 text-xs border border-slate-800 font-medium py-2 flex items-center justify-center gap-2 group cursor-pointer">
              <Link href="/admin/manage-inventory">
                Manage Availability <ArrowRight className="w-3.5 h-3.5 text-slate-500 group-hover:translate-x-1 transition-all" />
              </Link>
            </Button>
          </div>
        </Card>
