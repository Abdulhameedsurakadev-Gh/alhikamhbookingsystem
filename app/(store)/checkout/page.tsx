...
export default async function CheckoutPage(): Promise<React.JSX.Element> {
  ...
  // Pull down persistent cart matrix matched strictly to Better-Auth's user ID key framework
    // 🚀 FIXED: Removed the legacy user wildcard dependency to bypass client table conflicts
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5 flex items-center gap-1">
            <ShieldCheck className="h-3.5 w-3.5 text-emerald-700" /> Secure
            payment popup powered by Paystack
          </p>
        </div>
        <Link
          href="/cart"
          className="inline-flex items-center gap-1 text-xs font-bold text-slate-500 hover:text-emerald-800 transition"
        >
