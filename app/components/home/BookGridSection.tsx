...
export function BookGridSection({ title, title2, books }: BookGridSectionProps) {
  ...
  // Set instead of a single string — a single-id tracker only ever shows
  // "Added" on the most recently clicked book, silently reverting the
  // confirmation on every other book someone already added.
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {books.map((book) => {
            ...
            return (
              <div
                key={book.id}
                className={`bg-card border border-border rounded-md overflow-hidden flex flex-col group hover:shadow-subtle transition-all duration-fast ease-standard ${isOutOfStock ? "opacity-60" : ""}`}
              >
