...
import { subscribeToNewsletter } from "../../(store)/newsletter/actions";
          ...
          <div className="flex items-center gap-2 text-center md:text-left">
            ...
            <p className="text-label text-foreground">
              <span className="font-medium">
                New arrivals, quietly announced.
              </span>{" "}
              <span className="text-muted-foreground">
                No noise, just new titles as they arrive.
              </span>
            </p>
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 space-y-8">
        ...
        <div className="text-center md:text-left space-y-1.5 max-w-xs mx-auto md:mx-0">
          ...
          <p className="text-label text-muted-foreground leading-relaxed">
            Providing authentic Islamic books, translations, and scholarly works
            for students of knowledge throughout Ghana.
          </p>
        </div>

        <div className="flex items-center justify-center gap-4">
          <div className="h-px bg-border flex-1" />
          <div className="text-xs select-none text-muted-foreground font-serif">
            ❖
          </div>
          <div className="h-px bg-border flex-1" />
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 items-start">
          ...
          <div className="space-y-3 text-center sm:text-left">
            <h5 className="text-label font-semibold text-foreground font-serif">
              Catalogue
            </h5>
            ...
            <ul className="space-y-2 text-label text-muted-foreground">
              <li>
                <Link
                  href="/books"
                  className="hover:text-primary-hover transition-colors block"
                >
                  All Books
                </Link>
              </li>
        ...
        <div className="bg-card border border-border rounded-md p-4 flex flex-col md:flex-row items-center justify-between gap-4 text-center md:text-left">
          ...
          <div className="space-y-1">
            ...
            <p className="text-label font-medium text-foreground font-serif">
              Classical Authors & Commentators
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1.5 text-label text-muted-foreground">
            {scholars.map((sch, idx) => (
              <React.Fragment key={idx}>
                <Link
                  href={`/books?search=${encodeURIComponent(sch.query)}`}
                  className="hover:text-primary-hover inline-flex items-center gap-0.5 transition-colors"
                >
                  {sch.name}
                  <ArrowUpRight className="w-2.5 h-2.5" />
                </Link>

                {idx < scholars.length - 1 && (
        ...
        <div className="text-center text-xs text-muted-foreground tracking-wide pt-4 border-t border-border">
          &copy; {new Date().getFullYear()} Al-Hikmah Bookstore. Connecting
          students of knowledge with authentic Islamic books across Ghana.
        </div>
